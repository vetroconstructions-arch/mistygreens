...
(function() {
    ...
    const setupMagneticButtons = () => {
        const buttons = document.querySelectorAll('.btn-primary, .tour-btn, .pill-btn, .btn-submit');
        buttons.forEach(btn => {
            ...
            btn.addEventListener('mouseleave', () => {
                ...
                btn.style.transition = 'transform 0.5s var(--ease-editorial, cubic-bezier(0.16, 1, 0.3, 1))';
            });
        });
    };
...
// --- A11y: Focus Trapping and Keyboard Nav ---
document.addEventListener('keydown', (e) => {
    const activeModal = document.querySelector('.concierge-modal.active, .vault-modal.active, .plan-modal-overlay.active');
    if (!activeModal) return;

    if (e.key === 'Escape') {
        const closeBtn = activeModal.querySelector('.modal-close, #opening-close');
